...
PREVIEW_FILE_TOOL_DESCRIPTION = """Quickly preview the start of a file.

Usage:
- The file_path parameter must be an absolute path, not a relative path
- Reads the first N lines (default 50) starting from the beginning of the file
- Use this to sanity-check file contents before performing a full read
- Any lines longer than 2000 characters will be truncated
- Results are returned using cat -n format, with line numbers starting at 1"""
...
FILESYSTEM_SYSTEM_PROMPT = """## Filesystem Tools `ls`, `read_file`, `preview_file`, `write_file`, `edit_file`, `glob`, `grep`
...
def _preview_file_tool_generator(
    backend: BackendProtocol | Callable[[ToolRuntime], BackendProtocol],
    custom_description: str | None = None,
) -> BaseTool:
    """Generate the preview_file tool.
    ...
    Returns:
        Configured preview_file tool that reads the first N lines using the backend.
    """
    tool_description = custom_description or PREVIEW_FILE_TOOL_DESCRIPTION
    ...
    def preview_file(
        ...
        resolved_backend = _get_backend(backend, runtime)
        file_path = _validate_path(file_path)
        safe_limit = min(max(lines, 1), DEFAULT_READ_LIMIT)
        return resolved_backend.read(file_path, offset=0, limit=safe_limit)
...
TOOL_GENERATORS = {
    ...
    "preview_file": _preview_file_tool_generator,
...
class FilesystemMiddleware(AgentMiddleware):
    ...
    This middleware adds seven filesystem tools to the agent: ls, read_file, preview_file, write_file,
